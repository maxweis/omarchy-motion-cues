...
[[ $# -le 1 ]] || { echo 'Usage: render-demo.sh [output-directory]' >&2; exit 2; }
...
for motion_variant in showcase desktop; do
    ...
    QT_QPA_PLATFORM=offscreen QT_QUICK_BACKEND=software "$motion_stage/render" "$motion_stage/demo/Showcase.qml" "$motion_stage/$motion_variant" "${motion_options[@]}"
    ...
    ffmpeg -hide_banner -loglevel warning -y -framerate 30 -i "$motion_stage/$motion_variant/%04d.png" \
        ...
        -loop 0 "$motion_output/$motion_gif.gif"
    cp -- "$motion_stage/$motion_variant/0130.png" "$motion_output/$motion_variant.png"
done
printf 'Created showcase and example desktop media in %s\n' "$motion_output"
